...
async def start_blackjack_game(user, channel, bet):
    if user.id in blackjack_games:
        await channel.send("you already got a blackjack game running")
        return
    if get_wallet(user.id) < bet:
        await channel.send(f"ur broke, u got **{get_wallet(user.id)}** coins")
        return
    wallet[user.id] -= bet
    save_data()
    ...
    if game.check_starting_blackjack():
        view = PlayAgainView(user, channel, bet)
        await channel.send(f"{user.mention}", embed=game.build_embed(reveal=True), view=view)
        return
    view = BlackjackView(game)
    msg = await channel.send(f"{user.mention}", embed=game.build_embed(), view=view)
    view.message = msg
...
class PlayAgainView(discord.ui.View):
    ...
    async def replay_same(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(view=self)
        await start_blackjack_game(self.user, self.channel, self.last_bet)
    ...
    async def replay_new(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if new_bet <= 0:
                ...
                return
            await start_blackjack_game(self.user, self.channel, new_bet)
        except asyncio.TimeoutError:
            await self.channel.send(f"{self.user.mention} took too long to respond 💀")
...
class BlackjackView(discord.ui.View):
    ...
    async def finish_message(self):
        ...
        replay_view = PlayAgainView(self.game.user, self.game.channel, self.game.original_bet)
        await self.game.channel.send(view=replay_view)
...
async def blackjack(ctx, bet: int = None):
    ...
    if get_wallet(user.id) < bet:
        ...
        return
    await start_blackjack_game(user, ctx.channel, bet)
